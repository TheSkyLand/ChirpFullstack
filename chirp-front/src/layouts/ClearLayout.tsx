...
const Layout = ({ isAuthenticated, user, onLogout }: LayoutProps) => {
    ...
    // Функция для подсветки активных ссылок
    const getLinkStyle = (path: string) => {
        const isActive = location.pathname === path;
        return `flex items-center gap-4 p-3 rounded-full transition-all duration-200 ${
            isActive 
    ...
    return (
        <div className="min-h-screen bg-white">
                    ...
                    <div className="flex-1 flex justify-end gap-3">
                        {!isAuthenticated ? (
                            ...
                            <div className="flex items-center gap-4">
                                <Link to="/notifications" className="p-2 text-slate-600 hover:bg-gray-100 rounded-full transition-colors">
                                    <Bell size={24} />
                                </Link>
            ...
            <main className="max-w-[1400px] mx-auto pt-16 flex justify-between gap-4 px-4">
                ...
                {/* Левое боковое меню */}
                <aside className="hidden lg:flex flex-col w-64 shrink-0 sticky top-16 h-[calc(100vh-64px)] py-4 font-medium">
                    <nav className="flex flex-col h-full gap-1">
                        ...
                        {isAuthenticated && (
                            <>
                                ...
                                <Link to="/profile" className={getLinkStyle('/profile')}><User size={26} /> <span>Профиль</span></Link>

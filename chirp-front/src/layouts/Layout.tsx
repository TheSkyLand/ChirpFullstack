        ...
        <div className="min-h-screen bg-white">
            <header className="fixed top-0 w-full z-50 border-b bg-white/80 backdrop-blur-md h-16">
                <div className="max-w-[1400px] mx-auto h-full px-4 flex items-center">
                    ...
                    <div className="hidden md:flex flex-1 items-center relative w-full max-w-md">
                        <Search className="absolute left-4 text-gray-400" size={18} />
                        <input type="text" placeholder="Поиск по Chirp..." className="w-full bg-gray-100 rounded-full py-2.5 pl-12 pr-4 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                    </div>

                    <div className="flex-1 flex justify-end gap-3">
                        {!authStore.isAuthenticated ? (
                            <div className="flex gap-2">
                                <Link to="/login" className="px-4 py-2 font-bold text-slate-700 hover:text-blue-600">Войти</Link>
                                <Link to="/register" className="bg-blue-600 text-white px-5 py-2.5 rounded-full font-bold">Регистрация</Link>
                            </div>
                        ) : (
                            <div className="flex items-center gap-4">
                                <Link to="/notifications" className="p-2 text-slate-600 hover:bg-gray-100 rounded-full"><Bell size={24} /></Link>
